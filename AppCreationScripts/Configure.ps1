...
#> 
   ...
   Write-Host "Creating the AAD appplication (Webapp-Openidconnect)"
   # Get a 2 years application key for the app Application
   $pw = ComputePassword
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInYears 2 -pw $pw
   $appAppKey = $pw
   $appAadApplication = New-AzureADApplication -DisplayName "Webapp-Openidconnect" `
                                               -HomePage "https://localhost:8000/" `
											   -ReplyUrls "http://localhost:8080/adal4jsample/secure/aad" `
                                               -IdentifierUris "https://$tenantName/adal4j-sample" `
